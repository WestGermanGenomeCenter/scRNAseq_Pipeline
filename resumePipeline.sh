...
then
        ...
        snakemake --unlock --configfile $1 --use-conda --cluster-config cluster/cluster.json --cluster "qsub -A {cluster.account} -q {cluster.queue} -l select={cluster.nodes}:ncpus={params.cores}:mem={params.mem} -l walltime={params.time} -o {params.output} -e {params.error}" --jobs 20 -p --keep-going --jobscript cluster/jobscript.sh --latency-wait 180
fi
